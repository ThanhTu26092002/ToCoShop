...
router.patch("/updateOne/:id", validateId, async (req, res) => {
  try {
    ...
    //--Update in Mongodb
    const updatedDoc = await Transportation.findByIdAndUpdate(id, updateData, opts);
    if (!updatedDoc) {
      res.status(404).json({
        ok: true,
        error: {
    ...
    const errMsgMongoDB = formatterErrorFunc(errMongoDB, COLLECTION_TRANSPORTATIONS);
